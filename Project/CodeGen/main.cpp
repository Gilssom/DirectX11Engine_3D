...
int main()
{
	...
	wstring strCppPath = strProjPath + L"\\Scripts\\CScriptManager.cpp";
	wstring strHeaderPath = strProjPath + L"\\Scripts\\CScriptManager.h";
	...
	fwprintf_s(pFile, L"\tstatic CScript* GetScript(UINT scriptType);\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"CScript* CScriptManager::GetScript(UINT scriptType)\n{\n");

	fwprintf_s(pFile, L"\tswitch (scriptType)\n\t{\n");
	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		wstring strScriptUpperName = L"";
		for (UINT j = 1; j < g_vecName[i].size(); ++j)
