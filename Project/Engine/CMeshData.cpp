...
CMeshData* CMeshData::LoadFromFBX(const wstring& _RelativePath)
{
	...
	if (nullptr != pMesh)
	{
		...
		strMeshKey += L".mesh";
		CAssetManager::GetInst()->AddAsset<CMesh>(strMeshKey, pMesh);

		// 메시를 실제 파일로 저장
		pMesh->Save(CPathManager::GetInst()->GetContentPath() + strMeshKey);
	}
	...
	for (UINT i = 0; i < loader.GetContainer(0).vecMtrl.size(); ++i)
	{
		...
		Ptr<CMaterial> pMtrl = CAssetManager::GetInst()->FindAsset<CMaterial>(loader.GetContainer(0).vecMtrl[i].strMtrlName);
		assert(pMtrl.Get());
...
{
	wstring strRelativePath = CPathManager::GetInst()->GetRelativePath(_strFilePath);

	SetRelativePath(strRelativePath);

	wstring strFilePath = CPathManager::GetInst()->GetContentPath() + strRelativePath;
	...
	UINT i = 0;
	wstring strMtrlPath = CPathManager::GetInst()->GetContentPath();
