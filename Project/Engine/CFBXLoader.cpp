	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			for (size_t k = 0; k < vecPath.size(); ++k)
			{
				...
				CAssetManager::GetInst()->Load<CTexture>(path_dest, path_dest);
...
void CFBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			Ptr<CMaterial> pMaterial = CAssetManager::GetInst()->FindAsset<CMaterial>(strPath);
			...
			pMaterial->SetShader(CAssetManager::GetInst()->FindAsset<CGraphicShader>(L"Std3DDeferredShader"));

			wstring strTexKey = m_vecContainer[i].vecMtrl[j].strDiff;
			Ptr<CTexture> pTex = CAssetManager::GetInst()->FindAsset<CTexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexParam(TEX_PARAM::TEX_0, pTex);

			strTexKey = m_vecContainer[i].vecMtrl[j].strNormal;
			pTex = CAssetManager::GetInst()->FindAsset<CTexture>(strTexKey);
			if (NULL != pTex)
			...
			CAssetManager::GetInst()->AddAsset<CMaterial>(pMaterial->GetKey(), pMaterial.Get());
			pMaterial->Save(CPathManager::GetInst()->GetContentPath() + strPath);
		}
	}
}
